<?php if (!defined('THINK_PATH')) exit();?><html lang="zh-CN">
	<head>
		...
		<meta charset="utf-8" />
        <title><?php echo ($_SESSION['sheetname']); ?>--<?php echo (C("title")); ?></title>
...
<form class="form-horizontal" id="uniform" role="form" method="get" action="<?php echo ($postpage); ?>" >

<div class="form-group">
	<div class="controls col-xs-5">
            <select class="input-xlarge" name="sheetname" id="sheetname" >
                <option value="">所有表 中</option>
                <?php if(is_array($sheetnamearr)): $i = 0; $__LIST__ = $sheetnamearr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["sheetname"] == $slectsheet): ?><option value="<?php echo ($vo["sheetname"]); ?>" selected = "selected"><?php echo ($vo["sheetname"]); ?> 中</option>
                    <?php else: ?>    
                         <option value="<?php echo ($vo["sheetname"]); ?>"><?php echo ($vo["sheetname"]); ?> 中</option><?php endif; endforeach; endif; else: echo "" ;endif; ?>  
            </select>
    </div>
